...
export default function LeftPanel({
  ...
  const statusConfig = {
    registered: { color: 'bg-slate-500/20 text-slate-400 border-slate-500/30', label: 'Belum Dimulai',       dot: 'bg-slate-400' },
    started:    { color: 'bg-sky-500/20 text-sky-400 border-sky-500/30',       label: 'Sedang Dikerjakan',   dot: 'bg-sky-400 animate-pulse' },
    submitted:  { color: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30', label: 'Sudah Dikumpulkan', dot: 'bg-emerald-400' },
    locked:     { color: 'bg-red-500/20 text-red-400 border-red-500/30',       label: 'Waktu Habis',         dot: 'bg-red-400' },
  }[session.status]

  const timerColor =
    timerDanger  ? 'text-red-400 border-red-500/40 bg-red-500/10' :
    timerWarning ? 'text-amber-400 border-amber-500/40 bg-amber-500/10' :
    ...
    <aside className="h-full flex flex-col gap-3 overflow-y-auto scrollbar-thin pr-1">
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 flex-shrink-0">
        ...
        {session.startedAt ? (
          ...
          <>
            <div className="flex items-center justify-center px-3 py-3 border border-slate-700 rounded-xl bg-slate-700/30 mb-3">
            ...
            <p className="text-slate-600 text-[10px] text-center mt-2">Timer mulai saat tombol diklik</p>
          </>
        )}
      </div>
        ...
        {session.website_ujian ? (
          <>
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 py-2 bg-sky-500/10 hover:bg-sky-500/20 border border-sky-500/30 text-sky-400 text-sm font-semibold rounded-xl transition w-full"
            >
              Buka OpenCart ↗
            </a>
          </>
        ) : (
          <div className="flex items-start gap-2 py-2.5 px-3 bg-amber-500/10 border border-amber-500/20 rounded-xl">
            <span className="flex-shrink-0">⚠️</span>
            <p className="text-amber-400 text-xs">URL belum diisi. Hubungi dosen pengawas.</p>
          </div>
        )}
      </div>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 flex-1 min-h-0 overflow-hidden flex flex-col">
        ...
        {/* Overall bar */}
        <div className="h-2 bg-slate-700 rounded-full overflow-hidden mb-3 flex-shrink-0">
          <div
            ...
            return (
              <div key={cp} className={`flex items-center gap-2 px-2.5 py-2 rounded-lg text-sm transition-all ${
                ...
                <span className={`flex-1 truncate font-medium leading-tight ${
                  isDone ? 'text-emerald-400' : isAct ? 'text-sky-300' : 'text-slate-600'
                }`}>
                  {meta.label}
                </span>
        ...
        <div className="pt-2 border-t border-slate-700/50 mt-2 flex-shrink-0">
          <div className="flex items-center justify-between text-xs">
            <span className="text-slate-500">Total bobot</span>
            <span className="font-bold text-sky-400">100 poin</span>
          </div>
        </div>
      </div>
    </aside>
