...
export default function DataMahasiswa({ mahasiswaList, onRefresh }: DataMahasiswaProps) {
  ...
  const showMsg = (text: string, type: 'success' | 'error') => {
    ...
    setTimeout(() => setMessage({ text: '', type: '' as '' }), 5000)
  }
      ...
      try {
        ...
        try {
          ...
          data = text.split('\\\\n').filter(Boolean).map(line => {
            const [nim, nama, kelas, foto, website, aktif] = line.split(',').map(s => s.trim())
            return { nim, nama, kelas, foto, website_ujian: website, aktif: aktif !== 'false' && aktif !== '0' }
        ...
        const msg = typeof res === 'object' && res && 'message' in res ? String((res as { message: string }).message) : \`Import \${data.length} data berhasil\`
        showMsg(msg, 'success')
        onRefresh()
      } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      {message.text && (
        <div className={\`p-3 rounded-lg text-sm font-semibold \${message.type === 'success' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}\`}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={mhs.nim} className={\`border-t border-slate-700/50 hover:bg-slate-700/30 transition \${i % 2 === 1 ? 'bg-slate-800/50' : ''}\`}>
...
writeFileSync('src/components/dosen/DataMahasiswa.tsx', content, 'utf-8')
